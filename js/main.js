...
(function () {
  ...
  function initHeroVideo() {
    ...
    // Already buffered (bfcache / cached asset)
    ...
    // Try play (muted autoplay policies)
    const p = video.play();
    if (p && typeof p.catch === 'function') {
      p.catch(() => {
        // Autoplay blocked — still show poster/first frame if data is there
        if (video.readyState >= 2) markLoaded();
      });
    }
  }
    ...
    function renderCards() {
      ...
      grid.innerHTML = list
        .map((u) => {
          ...
          return `
          <article class="unit-card reveal is-visible status-${u.status}" data-unit="${u.id}" tabindex="0" role="button" aria-label="${t('res.unit')} ${u.id}">
            <div class="unit-image img-placeholder" data-label="${t('res.unit')} ${u.id}">
              <img src="${u.image}" alt="${t('res.unit')} ${u.id}" loading="lazy" class="w-full h-full object-cover" onerror="this.style.display='none'" />
